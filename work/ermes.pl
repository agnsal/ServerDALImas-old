...
eve(avvia):-print('Ermes in attesa!'),loop.

loop:-file_exists('notifica.txt')->sleep(0,2),modifica,avverti;loop.

modifica:-cancella,scrivi.

cancella:-open('dati.pl',write,var_W),write(var_W,''),close(var_W),delete_file('notifica.txt').

scrivi:-open('input.pl',read,var_Str),open('dati.pl',append,var_S),repeat,read(var_Str,var_T),print(var_T),write(var_S,var_T),write(var_S,'. '),(var_T==end_of_file->close(var_Str),close(var_S)).

avverti:-a(message(crono,send_message(aggiornamento,ermes))),a(message(psiche,send_message(aggiornamento,ermes))),a(message(atena,send_message(aggiornamento,ermes))),a(message(mnemosine,send_message(aggiornamento,ermes))).

eve(tempoelaborato(var_T)):-print('crono ha elaborato il tempo: '),print(var_T),print('. Invio dei dati al server!'),open('risposta.php',append,var_A),write(var_A,'$t='),write(var_A,var_T),write(var_A,'; '),close(var_A),open('c.txt',write,var_W),close(var_W),verifica.

eve(lavoroelaborato(var_L,var_X)):-print('Per la matricola '),print(var_X),print(', mnemosine ha trovato le mansioni: '),print(var_L),print('. Invio dei dati al server!'),open('risposta.php',append,var_A),write(var_A,'$l_'),write(var_A,var_X),write(var_A,'="'),write(var_A,var_L),write(var_A,'"; '),close(var_A),open('m.txt',write,var_W),close(var_W),verifica.

eve(testelaborato(var_F,var_X)):-print('Per la matricola '),print(var_X),print(' psiche ha calcolato felicita pari a: '),print(var_F),print('. Invio dei dati al server!'),open('risposta.php',append,var_A),write(var_A,'$f_'),write(var_A,var_X),write(var_A,'='),write(var_A,var_F),write(var_A,'; '),close(var_A),open('p.txt',write,var_W),close(var_W),verifica.

eve(misuraelaborato(var_S,var_X)):-print('Per la matricola '),print(var_X),print(' atena ritiene il suo atteggiamento al pc: '),print(var_S),print('. Invio dei dati al server!'),open('risposta.php',append,var_A),write(var_A,'$s_'),write(var_A,var_X),write(var_A,'='),write(var_A,var_S),write(var_A,'; '),close(var_A),open('a.txt',write,var_W),close(var_W),verifica.

verifica:-file_exists('a.txt'),file_exists('c.txt'),file_exists('m.txt'),file_exists('p.txt')->print('FATTO!'),notifica.

notifica:-delete_file('a.txt'),delete_file('c.txt'),delete_file('m.txt'),delete_file('p.txt'),open('notificaermes.txt',write,var_W),close(var_W),loop.
...
call_cfp(var_A,var_C,var_Ag):-clause(agent(var_AgI),var__),clause(ext_agent(var_Ag,_188046,var_Ontology,_188048),_188043),asserisci_ontologia(var_Ag,var_Ontology,var_A),once(call_meta_execute_cfp(var_A,var_C,var_Ag,_188065)),a(message(var_Ag,propose(var_A,[_188065],var_AgI))),retractall(ext_agent(var_Ag,_188084,var_Ontology,_188086)).

call_propose(var_A,var_C,var_Ag):-clause(agent(var_AgI),var__),clause(ext_agent(var_Ag,_187983,var_Ontology,_187985),_187980),asserisci_ontologia(var_Ag,var_Ontology,var_A),once(call_meta_execute_propose(var_A,var_C,var_Ag)),a(message(var_Ag,accept_proposal(var_A,[],var_AgI))),retractall(ext_agent(var_Ag,_188018,var_Ontology,_188020)).

call_propose(var_A,var_C,var_Ag):-clause(agent(var_AgI),var__),clause(ext_agent(var_Ag,_187927,var_Ontology,_187929),_187924),not(call_meta_execute_propose(var_A,var_C,var_Ag)),a(message(var_Ag,reject_proposal(var_A,[],var_AgI))),retractall(ext_agent(var_Ag,_187955,var_Ontology,_187957)).
...
call_cancel(var_A,var_Ag):-if(clause(high_action(var_A,var_Te,var_Ag),_187709),retractall(high_action(var_A,var_Te,var_Ag)),true),if(clause(normal_action(var_A,var_Te,var_Ag),_187726),retractall(normal_action(var_A,var_Te,var_Ag)),true).
...
refused_message(var_To,var_M):-clause(eliminated_message(var_M,var_To,motivation(conditions_not_verified)),_187617).
...
send_jasper_return_message(var_X,var_S,var_T,var_S0):-clause(agent(var_Ag),_187541),a(message(var_S,send_message(sent_rmi(var_X,var_T,var_S0),var_Ag))).

gest_learn(var_H):-clause(past(learn(var_H),var_T,var_U),_187515),learn_if(var_H,var_T,var_U).

evi(gest_learn(var_H)):-retractall(past(learn(var_H),_187453,_187454)),clause(agente(_187464,_187465,_187466,var_S),_187462),name(var_S,var_N),append(var_L,[46,112,108],var_N),name(var_F,var_L),manage_lg(var_H,var_F),a(learned(var_H)).

cllearn:-clause(agente(_187350,_187351,_187352,var_S),_187348),name(var_S,var_N),append(var_L,[46,112,108],var_N),append(var_L,[46,116,120,116],var_To),name(var_FI,var_To),open(var_FI,read,_187400,[]),repeat,read(_187400,var_T),arg(1,var_T,var_H),write(var_H),nl,var_T==end_of_file,!,close(_187400).
